...
def plot_mae_comparison(
    ...
    # Move legend to right-hand side to avoid overlapping tall bars
    ax.legend(
        fontsize=11,
        loc='center left',
    ...
    # numeric labels slightly above each bar (with adaptive offset)
    for bars in [bars1, bars2]:
        for bar in bars:
            height = bar.get_height()
            offset = 0.01 if height < 1 else 2.0  # small offset for SOC, larger for MF_IA/NOx
            ax.text(
...
if __name__ == "__main__":
    save_directory = "/Users/daisiqi/Machine-Learning-for-Thermodynamic-Property-dataset-URS-/virtual_sensor/figures_svr"
    os.makedirs(save_directory, exist_ok=True)

    df = pd.read_csv(
        '/Users/daisiqi/Machine-Learning-for-Thermodynamic-Property-dataset-URS-/virtual_sensor/df_processed.csv'
    )
